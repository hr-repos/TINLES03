...
class RobotMapper:
    def __init__(self):
        ...
        self.robot_x = 0
        self.robot_y = 0
        self.orientation = 0  # Needed for obstacle positioning
...
def on_connect(client, userdata, flags, rc, *args):
    if len(args) > 0:  # V2 has properties as 5th argument
        rc = args[0] if len(args) > 0 else rc
...
def on_message(client, userdata, msg):
    try:
        if msg.topic == "robot/position":
            ...
            x = data.get('x', 0)
            y = data.get('y', 0)
            print(f"Received position update: ({x}, {y})")
            mapper.update_position(x, y)
            
        elif msg.topic == "sensors/raw":
            data = json.loads(msg.payload.decode())
            print(f"Received sensor data: {data}")
            mapper.process_sensor_data(data)
...
try:
    ...
    print("Running visualization. Use WASD to move manually.")
